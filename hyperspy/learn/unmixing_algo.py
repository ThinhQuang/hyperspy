...
def blind_unmix(Y, R, Nmc = 100, geo_method = 'vca', bool_plot = 0):
    ...
    A_est = A_est/np.dot(np.sum(A_est, axis = 0)[:, np.newaxis], np.ones((1,R))).T
...
def sample_sigma2(A_est, M_est, Y):
    ...
    Tsigma2p = 1/np.random.gamma(coeff1, coeff2**(-1))
    ...
    if R == 1:
            ...
            for iter in range(5):
                    ...
                    for k in randperm:
                            ...
                            test = np.dot(Sigma_vect[k].T[:, np.newaxis], np.dot(Sigma_mat[k], Sk-Muk))
...
def sample_T_const(A,M,T,sigma2p,Tsigma2r,matU,Y_bar,Y,E_prior,bool_plot,y_proj):
    ...
    for r in randperm:
            ...
            for k in randperm1:
                    ...
                    vect_e = (-Y_bar - np.dot(matU[:, comp_k], tr[comp_k, 0]))/matU[:, k][:, np.newaxis]
                    ...
                    T_out[k,r] = dtrandn_MH(T_out[k,r], muk, math.sqrt(s2k), mum, mup)
...
def randnt(m, s, N):
    ...
    while (len(x) < NN):
        if (m < mA): #4. Exponential Distribution
            ...
            z = np.abs(np.random.random((N,1)))*s + m
            rho = (z>=0)
        elif m < mC: #2. Normal distribution coupled with the uniform ones
            r = ((np.random.random((N,1))) < m/(m+math.sqrt(math.pi/2)*s))
            u = np.random.random((N,1))*m
            g = np.abs(np.random.random((N,1))*s) + m
            z = r*u + (1-r)*g
